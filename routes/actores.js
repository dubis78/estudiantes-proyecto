...
router.get("/actor/:id", async(req,res) =>{
  const {id}= req.params;
  const [rows]=await cnn_mysql.execute(`SELECT * FROM actores WHERE id =${id}`);
  //!without const promiseConnection_mysql=connection_mysql.promise(); IN database we have to use: 
  //const [rows]=await cnn_mysql.promise().execute(`SELECT * FROM actores WHERE id =?`,[id]); 
...
router.get("/actores", async(req,res) =>{
  try {
    const [rows]=await cnn_mysql.execute(`SELECT * FROM actores`);
    res.json(rows);
...
//!without const promiseConnection_mysql=connection_mysql.promise(); IN database we can use:
//    cnn_mysql.execute(`SELECT * FROM actores`, (error,result,fields)=>{
//     if(error) {      
    ...
    try {
        if (Object.keys(req.body).length > 0) {
           ...
           // console.log(SQL, params)
            ...
            if (rows.affectedRows > 0) {
                [rows] = await cnn_mysql.query(`SELECT * FROM actores WHERE id = ?`, [id])
                res.json(rows[0])
            }else{
                res.json({})
            }
